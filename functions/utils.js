...
/**
 * Added by Adam Yarris
 */
const convert = (templateOriginal, ...inputs) => {
    let template = templateOriginal.map(x => x);

    template[0] = `<speak>${template[0]}`;
    let last = template.length - 1;
    template[last] = `${template[last]}</speak>`;

    return ssml(template, inputs).replace('IDE', '<say-as interpret-as="characters">IDE</say-as>');
};

module.exports = {ssml, convert};